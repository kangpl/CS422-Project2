...
class Task1Test extends FlatSpec {
  ...
  it should "give same results for medium dataset 4" in {
    val reducers = 4
    val cb = new CubeOperator(reducers)
  ...
  it should "give same results for medium dataset 8" in {
    val reducers = 8
    val cb = new CubeOperator(reducers)
  ...
  it should "give same results for medium dataset 12" in {
    val reducers = 12
    val cb = new CubeOperator(reducers)
  ...
  it should "give same results for medium dataset 16" in {
    val reducers = 16
    val cb = new CubeOperator(reducers)
  ...
  it should "give same results for medium dataset 20" in {
    val reducers = 20
    val cb = new CubeOperator(reducers)
  ...
  def inputSizeTest(cb: CubeOperator, dataset: Dataset, df: DataFrame, size: String) {
    ...
    val res_MRCube = timer { ("MRCube_" + size, cb.cube(dataset, groupingList, "lo_supplycost", op).count()) }
    val res_cubeNaive = timer { ("cubeNaive_" + size, cb.cube_naive(dataset, groupingList, "lo_supplycost", op).count()) }
