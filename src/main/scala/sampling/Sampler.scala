...
//    val lineitemRdd = lineitem.rdd
    ...
    val dfNewAgg = dfAgg.select(dfAgg("count(l_extendedprice)"), dfAgg("var_pop(l_extendedprice)"))
    val rddAgg = dfNewAgg.rdd
...
//    print(qcsWithKey.keys.distinct.count.toInt)
  ...
  def calError( x: Row, K: Double): Double = {
    var sampleSize = K
    val stratumSize = x.getDouble(0)
    if(stratumSize < K) sampleSize = stratumSize
    val error = scala.math.pow(stratumSize, 2)*x.getDouble(1)/sampleSize
    return error
  }
